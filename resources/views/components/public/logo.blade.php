...
@php
    $sizes = [
        'sm' => ['svg' => 'w-8 h-8', 'img' => 'h-8', 'text' => 'text-base', 'subtext' => 'text-[10px]'],
        'default' => ['svg' => 'w-10 h-10', 'img' => 'h-10', 'text' => 'text-lg', 'subtext' => 'text-xs'],
        'lg' => ['svg' => 'w-12 h-12', 'img' => 'h-12', 'text' => 'text-xl', 'subtext' => 'text-sm'],
    ];
    $s = $sizes[$size] ?? $sizes['default'];
    $customLogoUrl = \App\Models\GlobalSetting::getLogoUrl();
...
<div {{ $attributes->merge(['class' => 'flex items-center gap-3']) }}>
    @if($customLogoUrl)
        ...
        <img src="{{ $customLogoUrl }}" alt="{{ $siteName }}" class="{{ $s['img'] }} max-w-[200px] object-contain" />
    @else
