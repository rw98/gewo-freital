<div class="max-w-2xl">
    <flux:button variant="ghost" icon="arrow-left" :href="route('flats.show', $flat)" wire:navigate class="mb-4">
        {{ __('Back to Flat') }}
    </flux:button>

    <flux:heading size="xl" class="mb-2">{{ __('Edit Flat') }} {{ $flat->number }}</flux:heading>
    <flux:text class="mb-6">{{ $flat->rentalObject->fullAddress() }}</flux:text>
    ...
    <flux:card>
        <form wire:submit="save" class="space-y-6">
            ...
            <div class="flex items-center justify-end gap-4">
                <flux:button variant="ghost" :href="route('flats.show', $flat)" wire:navigate>
                    {{ __('Cancel') }}
                </flux:button>
