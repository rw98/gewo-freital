<div class="py-8">
    @if ($title || $description)
        <div class="text-center mb-8">
            ...
            @endif
            @if ($description)
                <flux:text class="mt-4 max-w-2xl mx-auto">{{ $description }}</flux:text>
            @endif
        </div>
    @endif

    {{-- Search Form --}}
    <form wire:submit="search" class="bg-zinc-50 rounded-2xl p-6 lg:p-8 max-w-4xl mx-auto">
    ...
    @if ($showFeatured)
        @if ($this->featuredListings->isNotEmpty())
            <div class="mt-12 grid md:grid-cols-{{ min($featuredCount, 3) }} gap-6">
                @foreach ($this->featuredListings as $listing)
                    <a href="{{ route('listings.show', $listing) }}" wire:key="featured-{{ $listing->id }}" class="group">
                        <flux:card class="h-full transition-shadow hover:shadow-lg">
                            <div class="aspect-video bg-linear-to-br from-zinc-100 to-zinc-50 rounded-lg mb-4 overflow-hidden">
                                ...
                                @else
                                    <div class="w-full h-full flex items-center justify-center">
                                        <flux:icon name="home" class="size-12 text-zinc-400" />
                                    </div>
                                @endif
                            </div>
        ...
        @else
            <div class="mt-12 text-center py-12">
                <flux:icon name="home" class="mx-auto size-16 text-zinc-300" />
        ...
        @if ($this->totalCount > $featuredCount)
